...
describe('Discover Component', () => {
  beforeEach(() => {
    // Mock Math.random to always return 0 for predictable template selection
  ...
  test('shows Peach Nectar notification for high match with correct template 1', () => {
    renderWithContext(
    ...
    // Template 1: "Sweet like Nectar! 🍯 You and {alias} are a {score}% match. You both hate {sharedDislike} but love {sharedLike}? Use 5 Pits to see your twin!"
    // sharedLike is picked from ["A", "B", "C"], since Math.random is 0, it picks first element (index 0).
    // So sharedLike="A", sharedDislike="X".
    // Score should be 100.

    expect(screen.getByText(/Sweet like Nectar! 🍯 You and HighMatch are a 100% match. You both hate X but love A\? Use 5 Pits to see your twin!/i)).toBeInTheDocument();
  });
