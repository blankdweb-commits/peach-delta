...
const Discover = ({ onNavigateToStore }) => {
  ...
  const calculateCompatibility = (user, match) => {
    ...
    return { score: Math.round(score), sharedLikes, sharedDislikes };
  };
  ...
  useEffect(() => {
    if (currentMatch) {
      ...
      // Cleanup previous notification when match changes or is ripened
      ...
      if (score >= 90) {
        ...
        const getRandom = (arr) => arr.length > 0 ? arr[Math.floor(Math.random() * arr.length)] : "something";

        if (rand < 0.33) {
           // Template 1
           const like = getRandom(sharedLikes);
           const dislike = getRandom(sharedDislikes);
           message = `Sweet like Nectar! 🍯 You and ${currentMatch.alias} are a ${score}% match. You both hate ${dislike} but love ${like}? Use 5 Pits to see your twin!`;
        } else if (rand < 0.66) {
           // Template 2
           const dislike = getRandom(sharedDislikes);
           message = `Shift Partner Alert! 🩺 A ${score}% match just landed ${currentMatch.distance}km away. They also hate ${dislike}. Ripen the connection now! 🍑`;
        } else {
           // Template 3
           message = `Is this your person? 😍 You and ${currentMatch.alias} have the same 'Sweet Peaches.' Don't let this one stay unripened!`;
        }
