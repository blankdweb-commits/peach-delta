...
const Discover = ({ onNavigateToStore }) => {
  ...
  const calculateCompatibility = (user, match) => {
    ...
    const sharedLikesList = user.likes.filter(l => match.likes.includes(l));
    const sharedDislikesList = user.dislikes.filter(d => match.dislikes.includes(d));

    const sharedLikesCount = sharedLikesList.length;
    const sharedDislikesCount = sharedDislikesList.length;

    // Weight likes higher?
    // 3 likes = 60 points (20 each)
    // 3 dislikes = 40 points (13.3 each)
    let score = (sharedLikesCount * 20) + (sharedDislikesCount * 13);
    if (score > 100) score = 100;

    return {
      score: Math.round(score),
      sharedLikes: sharedLikesList,
  ...
  useEffect(() => {
    if (currentMatch) {
      ...
      if (score >= 90 && !isRipped(currentMatch.id)) {
        const templates = [
          () => {
            const like = sharedLikes.length > 0 ? sharedLikes[0] : "the same things";
            const dislike = sharedDislikes.length > 0 ? sharedDislikes[0] : "the same things";
            return `Sweet like Nectar! 🍯 You and ${currentMatch.alias} are a ${score}% match. You both hate ${dislike} but love ${like}? Use 5 Pits to see your twin!`;
          },
          () => {
            const dislike = sharedDislikes.length > 0 ? sharedDislikes[0] : "bad vibes";
            return `Shift Partner Alert! 🩺 A ${score}% match just landed ${currentMatch.distance}km away. They also hate ${dislike}. Ripen the connection now! 🍑`;
          },
          () => `Is this your person? 😍 You and ${currentMatch.alias} have the same 'Sweet Peaches.' Don't let this one stay unripened!`
        ];

        const randomTemplateIndex = Math.floor(Math.random() * templates.length);
        const message = templates[randomTemplateIndex]();
