  ...
  useEffect(() => {
    if (currentMatch) {
      ...
      if (score >= 90 && !isRipped(currentMatch.id)) {
        ...
        // Template 1: Requires at least 1 shared like and 1 shared dislike
        if (sharedLikes.length > 0 && sharedDislikes.length > 0) {
          templates.push({
            message: `Sweet like Nectar! 🍯 You and ${currentMatch.alias} are a ${score}% match. You both hate ${sharedDislikes[0]} but love ${sharedLikes[0]}? Use 5 Pits to see your twin!`
          });
        }

        // Template 2: Requires at least 1 shared dislike
        if (sharedDislikes.length > 0) {
          templates.push({
            message: `Shift Partner Alert! 🩺 A ${score}% match just landed ${currentMatch.distance}km away. They also hate ${sharedDislikes[0]}. Ripen the connection now! 🍑`
          });
        }
