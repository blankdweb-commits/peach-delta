...
const Discover = ({ onNavigateToStore }) => {
  ...
  const getNotificationMessage = (data) => {
    const templates = [
      "Sweet like Nectar! 🍯 You and {alias} are a {score}% match. You both hate {sharedDislike} but love {sharedLike}? Use 5 Pits to see your twin!",
      "Shift Partner Alert! 🩺 A {score}% match just landed {distance}km away. They also hate {sharedDislike}. Ripen the connection now! 🍑",
      "Is this your person? 😍 You and {alias} have the same 'Sweet Peaches.' Don't let this one stay unripened!"
    ];

    const template = templates[Math.floor(Math.random() * templates.length)];

    // Helper to get random item from array or empty string
    const getRandom = (arr) => arr.length > 0 ? arr[Math.floor(Math.random() * arr.length)] : '';

    return template
      .replace('{alias}', data.alias)
      .replace('{score}', data.score)
      .replace('{distance}', data.distance)
      .replace('{sharedLike}', getRandom(data.sharedLikes))
      .replace('{sharedDislike}', getRandom(data.sharedDislikes));
  };
  ...
  useEffect(() => {
    if (currentMatch) {
      ...
      // Reset notification if match changes or conditions change
