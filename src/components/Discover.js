  ...
  useEffect(() => {
    if (currentMatch) {
      ...
      if (score >= 90 && !isRipped(currentMatch.id)) {
        const templates = [
          `Sweet like Nectar! 🍯 You and ${currentMatch.alias} are a ${score}% match. You both hate ${sharedDislikes[0] || 'bad vibes'} but love ${sharedLikes[0] || 'good vibes'}? Use 5 Pits to see your twin!`,
          `Shift Partner Alert! 🩺 A ${score}% match just landed ${currentMatch.distance}km away. They also hate ${sharedDislikes[0] || 'bad vibes'}. Ripen the connection now! 🍑`,
